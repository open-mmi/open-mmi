    ...
    for name, raw_meta in raw_buses.items():
        ...
        if physical_can and (
            ...
            raise ValueError(
                ...
                "udev private-namespace provisioning"
            )
...
def render_systemd_dropin(plan: ProfileProvisionPlan) -> str:
    receive_line = ""
    if (
        plan.active_interface.startswith("can")
        and plan.active_interface[3:].isdigit()
        and 1 <= len(plan.active_interface[3:]) <= 3
    ):
        receive_line = 'Environment="OPEN_MMI_CAN_RECEIVE_INTERFACE=openmmi-rx"\n'
...
def render_udev_rules(plan: ProfileProvisionPlan) -> str:
    ...
    for bus in plan.buses:
        ...
        lines.append(
            ...
            f'RUN+="/sbin/ip link set {bus.interface} down", TAG+="systemd", '
