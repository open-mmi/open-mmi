...
- Local-only `GET /api/system/vehicle-setup/coordinator`, `POST /api/system/vehicle-setup/preview`, fixed confirmed `POST /api/system/vehicle-setup/apply`, and matching status/preview clients. The Settings apply button remains disabled while the UI workflow is built; caller-selected restore remains unavailable.
