...
// --- Open MMI stable footer tell-tales start ---
(function installStableFooterTelltales() {
  "use strict";
  ...
  const HEADLAMP_SVG = `
    <svg viewBox="0 0 64 64" aria-hidden="true" focusable="false">
      <path d="M28 13c11 0 20 8.5 20 19s-9 19-20 19H18c-3.3 0-6-2.7-6-6V19c0-3.3 2.7-6 6-6h10z" fill="none" stroke="currentColor" stroke-width="5" stroke-linejoin="round"/>
      <path d="M31 18c5 2.9 8 7.8 8 14s-3 11.1-8 14" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round"/>
      <path d="M52 19h8M52 29h8M52 39h8M52 49h8" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round"/>
    </svg>`;

  const HIGH_BEAM_SVG = `
    <svg viewBox="0 0 64 64" aria-hidden="true" focusable="false">
      <path d="M28 13c11 0 20 8.5 20 19s-9 19-20 19H18c-3.3 0-6-2.7-6-6V19c0-3.3 2.7-6 6-6h10z" fill="none" stroke="currentColor" stroke-width="5" stroke-linejoin="round"/>
      <path d="M52 16h9M52 24h9M52 32h9M52 40h9M52 48h9" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round"/>
    </svg>`;
  ...
  const SLOT_DEFS = [
    ...
    { key: "lights", label: "Exterior lights", kind: "headlamp", svg: HEADLAMP_SVG },
    { key: "highbeam", label: "High beam", kind: "high-beam", svg: HIGH_BEAM_SVG },
  ];
  ...
  function parseForcedTokens() {
    ...
    const aliases = new Map([
      ...
      ["hazards", "hazard"],
      ["lamp", "lights"], ["light", "lights"], ["headlamp", "lights"], ["headlight", "lights"], ["headlights", "lights"],
      ...
      ["hot", "coolant"], ["overheat", "coolant"], ["overheat-coolant", "coolant"],
      ["doors", "door"], ["dooropen", "door"], ["door-open", "door"],
      ["all", "all"],
    ]);

    const tokens = new Set();
    raw.split(/[\s,;+|]+/).map((token) => token.trim().toLowerCase()).filter(Boolean).forEach((token) => {
      const clean = token.replace(/_/g, "-");
      tokens.add(aliases.get(clean) || aliases.get(clean.replace(/-/g, "")) || clean);
    });
    return tokens;
  }
  ...
  function applyTestOverrides(payload) {
    ...
    if (forced("lights")) {
      ...
      lighting.mode = lighting.mode && String(lighting.mode).toLowerCase() !== "off" ? lighting.mode : "on";
    }
    if (forced("highbeam")) {
      lighting.lights_on = true;
    ...
    if (forced("door")) {
      state.doors.any_open = true;
      state.doors.driver_open = true;
    }
    if (forced("voltage")) {
      state.electrical.voltage_v = 11.4;
      state.electrical.low_voltage = true;
    }
    if (forced("coolant")) {
      state.engine.coolant_temp_c = 118;
      state.engine.coolant_warning = true;
    }
  ...
  function lightsAreOn(lighting) {
    if (truthy(lighting.lights_on)) return true;
    const mode = String(lighting.mode || "").trim().toLowerCase();
    if (!mode || ["off", "none", "unknown", "--"].includes(mode)) return false;
    return /light|lamp|beam|side|dipped|head|auto|park/.test(mode);
  }

  function highBeamIsOn(lighting) {
    return truthy(lighting.high_beam) || truthy(lighting.main_beam) || truthy(lighting.full_beam) || /high|main|full/.test(String(lighting.mode || "").toLowerCase());
  }

  function highBeamKnown(lighting) {
    return Object.prototype.hasOwnProperty.call(lighting, "high_beam") ||
      Object.prototype.hasOwnProperty.call(lighting, "main_beam") ||
      Object.prototype.hasOwnProperty.call(lighting, "full_beam") ||
      /high|main|full/.test(String(lighting.mode || "").toLowerCase()) ||
      forced("highbeam");
  }
  ...
  function ensureStrip() {
    ...
    if (!strip.dataset.openMmiStableSlots) {
      strip.innerHTML = SLOT_DEFS.map((slot) => slotMarkup(slot)).join("");
      strip.dataset.openMmiStableSlots = "1";
    }
  ...
  function setSlot(strip, key, active, known, label) {
  ...
  function hideMovedPageTelltales() {
    const selectors = [
      ...
      '[data-bool="handbrake"]',
      '[data-bool="hazards"]',
      '[data-bool-no="bulb_out"]',
      '[data-field="lights_on"]',
      '[data-field="lighting_mode"]'
    ];
    ...
    selectors.forEach((selector) => {
      document.querySelectorAll(selector).forEach((node) => {
        ...
        host.classList.add("openmmi-telltale-moved-to-footer");
        host.classList.add("openmmi-light-state-moved-to-footer");
  ...
  function hideOldFooterItems() {
    ...
    footer.querySelectorAll(".footer-item, .status-item, .tile, article, li").forEach((node) => {
      ...
      if (/(reverse|handbrake|parking brake|park brake|parking|brake)/.test(text)) {
  ...
  function updateTestBadge() {
    ...
    const tokens = Array.from(new Set([...FORCED_FROM_URL, ...forcedFromKeyboard])).join(" ");
    badge.textContent = `TEST ${tokens}`;
  }
  ...
  function renderStableFooterTelltales(payload) {
    ...
    const left = hazards ? true : lighting.left_indicator;
    const right = hazards ? true : lighting.right_indicator;
    const lightsOn = lightsAreOn(lighting);
    const highBeam = highBeamIsOn(lighting);
    const highKnown = highBeamKnown(lighting);

    setSlot(strip, "left", left === true, left !== undefined && left !== null, "Left indicator");
    setSlot(strip, "right", right === true, right !== undefined && right !== null, "Right indicator");
    setSlot(strip, "park", vehicle.handbrake === true, vehicle.handbrake !== undefined && vehicle.handbrake !== null, "Parking brake");
    setSlot(strip, "hazard", lighting.hazards === true, lighting.hazards !== undefined && lighting.hazards !== null, "Hazard warning");
    setSlot(strip, "bulb", lighting.bulb_out === true, lighting.bulb_out !== undefined && lighting.bulb_out !== null, "Exterior bulb failure");
    setSlot(strip, "lights", lightsOn === true, true, "Exterior lights");
    setSlot(strip, "highbeam", highBeam === true, highKnown, "High beam");
  ...
  function scheduleStableFooterTelltales(payload) {
    ...
    renderRaf = requestAnimationFrame(() => {
      renderRaf = 0;
      try {
        renderStableFooterTelltales(lastPayload || {});
      } catch (error) {
        console.warn("Open MMI stable footer tell-tales failed", error);
      }
    });
  }
  ...
  function toggleForcedToken(token) {
    ...
    if (window.openMmiLastStatusPayload && typeof render === "function") {
      render(window.openMmiLastStatusPayload);
    } else {
      scheduleStableFooterTelltales(lastPayload);
    }
  }
  ...
  if (typeof render === "function" && !render.__openMmiStableFooterTelltalesWrapped) {
    const previousRender = render;
    render = function renderWithStableFooterTelltales(payload) {
    ...
    render.__openMmiStableFooterTelltalesWrapped = true;
  }
...
// --- Open MMI stable footer tell-tales end ---
