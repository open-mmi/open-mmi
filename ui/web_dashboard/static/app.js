...
(function openMmiMediaSourceShellV1() {
  ...
  function shouldUseJellyfin() {
    ...
    return ["jellyfin", "radio", "usb"].includes(active) && isEnabled(active, prefs);
  }
  ...
  function sourceToggleRow(source, prefs) {
    ...
    return settingsRow(
      source.label,
      source.planned ? `${source.note}; can be exposed as a placeholder source.` : source.id === "usb" ? "Read-only local media roots configured or discovered by the dashboard server." : "Configured server-side with URL/token environment variables.",
...
(function openMmiMediaSourceAdaptersRadio() {
  ...
  function applySourceUi(adapter = activeAdapter()) {
    ...
    root.classList.remove("openmmi-media-source-jellyfin", "openmmi-media-source-radio", "openmmi-media-source-usb");
